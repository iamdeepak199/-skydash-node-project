...
router.get('/Rental_Manager', async (req, res) => {
    try {
        ...
        const [results] = await db.query(`SELECT * FROM roi_payout_log LIMIT ? OFFSET ?`, [limit, offset]);

        // Query to get the total count of records
        const [countResult] = await db.query('SELECT COUNT(*) AS count FROM roi_payout_log');
