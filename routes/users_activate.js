...
router.get('/users_activate', async (req, res) => {
    try {
        ...
        const [results] = await db.query(`SELECT * FROM users_activate LIMIT ? OFFSET ?`, [limit, offset]);

        // Query to get the total count of records
        const [countResult] = await db.query('SELECT COUNT(*) AS count FROM users_activate');
        const totalItems = countResult[0].count;
        const totalPages = Math.ceil(totalItems / limit);  // Calculating total pages

        // Render the EJS view and pass the data
        res.render('users_activate', {
            users_activate: results,
            currentPage: page,
            totalPages,
